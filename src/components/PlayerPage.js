...
const PlayerPage = () => {
  ...
  const [stats, setStats] = useState({ games: { won: 0, lost: 0, draw: 0 }, hands: { rock: 0, paper: 0, scissors: 0 } })
  ...
  const calculateStats = (gameArray, name) => {
    let won = 0;
    let lost = 0;
    let draw = 0;
    let rock = 0;
    ...
    setStats({ games: { won, lost, draw }, hands: { rock, paper, scissors }, winratio, favouriteHand })
  }
  ...
  if (isLoading) {
    ...
    return (
      <div className="player-page">
        <h3>
          Could not find player with the name {name}
        </h3>
      </div>
    )
  } else {
